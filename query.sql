...
-- 8- Selezionare tutte le recensioni con voto compreso tra 2 e 4 (2947)
    ...
    SELECT * FROM `reviews` WHERE `rating` > 1 AND `rating` < 5;
...
-- 11- Selezionare il numero e la media delle recensioni per il videogioco con ID = 412 (review number = 12, avg_rating = 3.16 circa)
    ...
    SELECT COUNT(*) AS numero_recensioni, AVG(`rating`) AS media_recensioni FROM `reviews` WHERE `videogame_id` = 412;
...
-- 2- Sezionare tutti i videogame dei tornei tenuti nel 2016, mostrandoli una sola volta (226)
    ...
    SELECT DISTINCT `videogames`.* FROM `videogames` JOIN `tournament_videogame` ON `videogames`.`id` = `tournament_videogame`.`videogame_id` JOIN `tournaments` ON `tournament_videogame`.`tournament_id` = `tournaments`.`id` WHERE `tournaments`.`year` = 2016;

-- 3- Mostrare le categorie di ogni videogioco (1718)

    SELECT `videogames`.`id` AS `id_videogioco`, `categories`.`name` AS `categoria` FROM `videogames` JOIN `category_videogame` ON `videogames`.`id` = `category_videogame`.`videogame_id` JOIN `categories` ON `category_videogame`.`category_id` = `categories`.`id`;
...
-- 5- Selezionare i premi ricevuti da ogni software house per i videogiochi che ha prodotto (55)
    ...
    SELECT `software_houses`.`name` AS `softwarehouse_name`, `awards`.`name` AS `award` FROM `software_houses` JOIN `videogames` ON `software_houses`.`id` = `videogames`.`software_house_id` JOIN `award_videogame` ON `videogames`.`id` = `award_videogame`.`videogame_id` JOIN `awards` ON `award_videogame`.`award_id` = `awards`.`id`;
